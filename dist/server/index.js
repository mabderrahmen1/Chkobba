 ...
 * @returns {Game}
 */
function getOrCreateGame(roomId, room) {
    if (!games.has(roomId)) {
...
io.on('connection', (socket) => {
     ...
     * Create a new room
     */
    socket.on('create_room', ({ nickname, targetScore, maxPlayers }) => {
    ...
    socket.on('rejoin_room', ({ roomId, nickname }) => {
        try {
            ...
            const disconnectedPlayer = room.players.find(p => !p.isConnected && p.nickname.toLowerCase() === nickname.toLowerCase());
