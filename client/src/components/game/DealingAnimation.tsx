  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[80] flex items-center justify-center">
      ...
      <motion.div
        ...
        animate={{ scale: 1 }}
        exit={{ scale: 0 }}
        className="w-[70px] h-[95px] bg-surface-2 rounded-lg border border-border shadow-md flex items-center justify-center"
      >
        <div className="text-text-tertiary text-[8px] uppercase text-center px-1">Dealing...</div>
      </motion.div>
    </div>
  );
}
...
function DealingCard({ item, index, players, myId, onAppear, timePerCard }: { item: any; index: number; players: any[]; myId: string | null; onAppear: () => void; timePerCard: number }) {
  ...
  return (
    <motion.div
      ...
      className="absolute w-[60px] h-[84px] bg-white rounded-lg border border-border shadow-lg overflow-hidden"
    >
...
function getPlayerPosition(playerIndex: number, players: any[], myId: string | null) {
  ...
  if (players.length >= 4) {
    if (targetPlayer.team === me.team) return { x: 0, y: -offset };
